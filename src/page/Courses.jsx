...
export default function Courses() {
  ...
  const [uploadedMaterials, setUploadedMaterials] = useState([]); // เก็บไฟล์ชีทเรียนที่แอดมินอัปโหลด
    ...
    const promises = files.map(file => {
      return new Promise((resolve) => {
        ...
        reader.onloadend = () => {
          resolve({
            name: file.name,
            data: reader.result // ข้อมูลไฟล์ในรูปแบบ Base64 Data URL
          });
        };
        reader.readAsDataURL(file);
      });
  ...
  // --- BACKOFFICE ADMIN CONTROL PIPELINES ---
      ...
      {!selectedCourse ? (
        ...
        <div className="max-w-7xl mx-auto px-4 py-10 space-y-8">
          ...
          <div className="max-w-md bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-2xl p-2 flex items-center gap-2 shadow-sm focus-within:border-blue-500 transition-colors">
            ...
            <input 
              type="text" placeholder="ค้นหาคอร์สเรียนกวดวิชา, แนวคิด หรือสารบัญวิชา..." value={searchQuery}
